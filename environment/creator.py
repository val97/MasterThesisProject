...
class Document(document.AbstractDocument):
  ...
  def create_observation_nominal(self):
    return {
       'doc_id': int(self._doc_id),
       'topic': np.argmax(self.topic, axis=0),
       'creator_id': int(self.creator_id),
     }
...
class Creator:
  ...
  def create_observation(self):
    return {
        ...
        'creator_topic_preference': self.topic_preference
    }
...
class DocumentSampler(document.AbstractDocumentSampler):
  ...
  @property
  def get_viable_creators(self):
     return self.viable_creators
