...
def save_summary(summary_writer,
  ...
  with summary_writer.as_default():
    ...
    """for top in range(10):
        print(np.array(topic_distribution)[:,top].shape, np.array(topic_distribution)[:,top])

        for s in range(20):
            tf.summary.histogram('topic distribution', np.array(topic_distribution)[:,top], step=s)
    print(np.array(topic_distribution).shape, np.array(topic_distribution))"""

#   tf.summary.histogram("prova",np.array([[0,0,0,0,0,0,5,0,1,1],[0,1,2,2,2,5,10,1,1,1]])[:,epoch], step = epoch)
